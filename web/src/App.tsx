...
function App() {
  ...
  useEffect(() => { sessionScopeRef.current = currentSessionId ?? ""; }, [currentSessionId]);
      ...
      <main className="project-center">
        ...
        <section className="project-grid">
          ...
          <form className="project-card add-project" onSubmit={trust}>
            ...
            <input value={projectsFeature.trustPath} onChange={(event) => projectsFeature.setTrustPath(event.target.value)} placeholder="/path/to/repository" />
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <button className="project-picker" onClick={() => setShowProjectPicker((value) => !value)} aria-expanded={showProjectPicker} title={projectsFeature.activeProject?.primary_workspace_root}>
          <span>{projectsFeature.activeProject ? shortPath(projectsFeature.activeProject.primary_workspace_root) : "选择项目"}</span>
        ...
        <button className={`plan-toggle ${showPlan ? "active" : ""}`} onClick={() => setShowPlan((value) => !value)} aria-pressed={showPlan} aria-label={showPlan ? "隐藏计划" : "显示计划"}><PlanIcon />{planningFeature.planning.plan?.status === "proposed" && <i className="plan-badge" title="有计划待评审" />}</button>
      ...
      <div className={`workspace-grid ${showPlan ? "with-plan" : ""}`} style={{ "--context-width": `${planWidth}px` } as CSSProperties}>
        <aside className="sidebar">
          ...
          <div className="workspace-roots"><span>工作区</span>{projectsFeature.activeProject?.workspace_roots.map((root) => <small key={root} title={root}>● {root}</small>)}</div>
        </aside>
        ...
        <section className="conversation">
          <div className="timeline" ref={timelineRef} onScroll={handleTimelineScroll}>
            ...
            {transcript.map((item) => {
              ...
              if (run) return <RestoredTurnView run={run} item={item} directory={admin.toolDirectory} key={item.event_id} />;
              return <Message item={item} key={item.event_id} />;
            })}
            {localTurns.map((turn) => <LocalTurnView turn={turn} directory={admin.toolDirectory} key={turn.clientId} />)}
          </div>
          <div className="conversation-footer">
            {showJumpToBottom && <button className="jump-bottom" onClick={jumpToBottom}>回到底部 ↓</button>}
            {promptsFeature.prompts[0] && <PromptCard
              ...
              prompt={promptsFeature.prompts[0]} onResolve={promptsFeature.resolve} />}
      ...
      {showSettings && <SettingsPanel
        ...
        providerSettings={admin.providerSettings} providerFields={admin.providerFields} modelFields={admin.modelFields}
        providers={admin.providers} knownProviders={admin.knownProviders} llmRuntime={admin.llmRuntime}
        catalog={{
          currentModel: admin.currentModel, overrides: admin.overrides, origins: admin.origins,
          thinking: admin.thinking, tools: admin.tools, status: admin.statusView, recovery: admin.recovery,
        }}
        actions={{
        ...
        onClose={() => setShowSettings(false)} onSave={settingsFeature.save} onReset={settingsFeature.reset} onAddRoot={admin.addWorkspace}
        onRemoveRoot={admin.removeWorkspace} onRevokeRule={admin.revokeRule} onRestore={admin.restoreCheckpoint}
        onAddModel={admin.addModel} onRemoveModel={admin.removeModel} onSaveModel={admin.saveModelFields}
        providerProtocols={admin.providerProtocols} onAddProvider={admin.addProvider}
        onSaveProvider={admin.saveProviderFields} onSaveLlmRuntime={admin.saveLlmRuntime} />}
    </main>
  );
}
