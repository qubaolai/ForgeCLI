          ...
          fields={[
            {
              key: "cache.enabled",
              label: "缓存开关",
              value: String(admin.llmRuntime.cache.enabled),
              choices: ["false", "true"],
            },
            {
              key: "cache.ttl_seconds",
              label: "缓存 TTL（秒）",
              value: String(admin.llmRuntime.cache.ttl_seconds ?? ""),
            },
            {
              key: "cache.max_entries",
              label: "缓存条目上限",
              value: String(admin.llmRuntime.cache.max_entries),
            },
            { key: "cache.origins", label: "缓存 origins", value: admin.llmRuntime.cache.origins.join(",") },
            {
              key: "circuit_breaker.enabled",
              label: "熔断开关",
              value: String(admin.llmRuntime.circuit_breaker.enabled),
              choices: ["false", "true"],
            },
            {
              key: "circuit_breaker.failure_threshold",
              label: "失败阈值",
              value: String(admin.llmRuntime.circuit_breaker.failure_threshold),
            },
            {
              key: "circuit_breaker.cooldown_seconds",
              label: "冷却时间（秒）",
              value: String(admin.llmRuntime.circuit_breaker.cooldown_seconds),
            },
            {
              key: "retry.wait_threshold_seconds",
              label: "429 等待阈值（秒）",
              value: String(admin.llmRuntime.retry.wait_threshold_seconds),
            },
          ]}
