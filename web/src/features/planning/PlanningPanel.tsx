  ...
  if (!plan && !todo && !others.length) {
    return (
      <div className="panel-empty">
        <span>◇</span>
        <p>当前没有活动计划</p>
        <small>切换到 Plan 模式，让 Forge 先梳理实施方向。</small>
      </div>
    );
  }
  return (
    <div className="plan-panel">
        ...
        <>
          ...
          {plan.status === "proposed" && (
            <div className="plan-actions">
              <button onClick={() => onResolve("reject")}>拒绝</button>
              <button onClick={() => onResolve("approve")}>同意</button>
              <button className="primary" onClick={() => onResolve("approve_and_run")}>
                同意并执行
              </button>
            </div>
          )}
      ...
      {todo?.items?.length ? (
        <section className="todo-block">
          ...
          <ol className="todo-list">
            {todo.items.map((item, position) => (
              <li className={`todo-${item.status}`} key={`${position}-${item.title}`}>
                <i />
                {item.title}
              </li>
            ))}
      ...
      {others.length > 0 && (
        <section className="plan-catalog">
          <h4>其他计划</h4>
          {others.map((item) => (
            <button className="plan-entry" key={item.plan_id} onClick={() => onActivate(item.plan_id)}>
              <strong>{item.title || item.plan_id}</strong>
              <small>
