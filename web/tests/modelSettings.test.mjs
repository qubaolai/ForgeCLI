...
test("sampling parameters are always sent, never left to the provider", () => {
  ...
  assert.deepEqual(buildInitialModelParams("64000", "8192"), {
    context_window: 64000, max_tokens: 8192, temperature: 0.7, top_p: 1, thinking_mode: "off",
  });
});

test("thinking is part of adding a model, not a later separate edit", () => {
  assert.deepEqual(
    buildInitialModelParams("", "", { temperature: "0.2", topP: "0.9", thinkingMode: "on", thinkingEffort: "high" }),
...
test("an out-of-range sampling value is refused before it reaches the backend", () => {
  assert.throws(
    () => buildInitialModelParams("", "", { temperature: "3", topP: "1.0", thinkingMode: "off", thinkingEffort: "" }),
    /温度必须是 0 到 2 之间的数字/,
  );
  assert.throws(
    () => buildInitialModelParams("", "", { temperature: "0.7", topP: "2", thinkingMode: "off", thinkingEffort: "" }),
