...
```text
PromptBlock
  block_id                 # core_identity / tool_contract / workspace_instructions / runtime_facts
  heading                  # 渲染出来的小节标题
  body                     # 该块正文
...
| 2 | `tool_contract` | 工具, 真实性, 审批与交互契约 | 结构性 | 是 |
| 3 | `workspace_instructions` | 项目指令 `FORGE.md` | 条件性 | 是 |
| — | — | **Prompt Cache 断点** | — | — |
| 4 | `runtime_facts` | 当前运行事实 | 结构性 | 否 |
...
**结构性块**无条件渲染, 即使内容为空也保留标题. `core_identity` 与 `tool_contract`
永远有内容; `runtime_facts` 永远至少有 mode 与 platform.
