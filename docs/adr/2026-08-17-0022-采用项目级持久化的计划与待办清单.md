...
### 1. 计划与待办是**会话级**持久化状态, 不是会话内存态

> **2026-08-19 修订.** 本条原文写的是"项目级, 跨会话". 实现之后推翻了, 理由见下方
> 「为什么不跨会话」. 备选方案 A 当初被否掉的理由随之作废, 那一节也一并改了.

落盘位置:

```text
~/.forge/projects/<project_id>/sessions/<session_id>/plans/
```

**落盘**是为了扛住进程退出与崩溃 —— 一次评审停顿要跨工具调用边界, 计划必须在磁盘上.
**按会话分区**是为了不跨对话.

#### 为什么不跨会话

跨会话带出一份计划或一份待办, 等于带出一份**没有上下文的清单**.

一条写着"切分"的待办, 切什么, 按什么边界 —— 那个信息在产生它的对话里, 不在清单里. 新
会话读到它只会按自己的理解填空, 而那个理解未必是当初的. 这是**看不出来的漂移**: 清单
本身长得完全正常, 没有任何一层会报错.

"中断之后接着干"这个需求不需要靠跨会话持久化来满足: `/resume` 回到的是**同一个**
`session_id`, 历史由事件重建 (`AgentTurnService.resume`), 上下文跟着回来, 计划与待办也
就还在原处.

住在会话目录下而不是在项目目录里加一个 `session_id` 字段再过滤: 如果它只对一个会话有
意义, 就该住在那个会话里. 附带好处是删会话时它一起走, 项目目录不会攒下孤儿计划.

仍然不落在工作区里 —— 那会污染用户仓库, 还会被下一轮 Agent 当成项目内容读回上下文
(与 `state_dir` 同一条理由); 落在工作区外的另一个后果是 `fs.*` 工具够不到它, 而那正是
决策 5 想要的隔离.
...
既有先例是 `JsonLearnedRuleStore`: 学习规则同样是项目级, 同样按项目单独存储, 同样不进
...
### 10. `plan_id` 与 `todo_id` 由模型按任务命名

> **2026-08-20 修订.** 原实现用 `pl_a3f19c` 这样的随机短 id, 理由是"标题会改, 而 id 是
> 文件名的一部分". 那个理由只证明了**不该由标题派生**, 没有证明**必须随机**.

`plan.write` 与 `todo.write` 各加一个必填的 `name`: 模型按这次任务给它起个短名, Forge
规整成目录名. 于是 `~/.forge/.../plans/` 下一眼能看出每份计划是关于什么的, `/plan list`
与索引也不再是一串对不上号的十六进制.

三条约束把"能改的东西"和"不能改的东西"分开:

- **只在新建时取一次.** 后续 revision 沿用同一个 id, 改标题不会让历史散成两份 —— 原来
  那条理由在这里仍然成立, 所以 `name` 在续写时被忽略.
- **撞名加序号, 不覆盖.** id 是目录名, 两份同名计划直接覆盖等于丢掉上一份的全部 revision.
- **规整而不是过滤.** 保留 Unicode 字词字符 (中文名字要能用), 剔除的是路径分隔符, 控制
  字符与保留名; 规整之后为空才退回随机 id —— 那种名字当目录名是危险的, 不是不好看.

待办同名表示继续修正当前这份清单 (revision + 1), 换名表示这是另一件事的清单 (从
revision 1 起算, 旧的进归档). `seed_from_plan` 沿用计划的名字: 同一件事在磁盘上就是同
一个名字.
...
> **2026-08-19: 本方案已改为采用.** 原文否掉它的理由是"跨会话就丢, 而计划的价值恰恰在
> 跨会话 —— 一个需要计划的任务多半做不完一个会话".
>
> **那条理由是错的**, 两处:
>
> 1. "做不完一个会话"混淆了会话与进程. `/resume` 回到的是同一个 `session_id`, 计划不会
>    丢 —— 会丢的只是**换一段全新对话**时, 而那时本来也不该带出旧计划.
> 2. 跨会话带出的是一份没有上下文的清单. 计划的价值不在"能被带到别处", 而在"它与产生它
>    的那段对话是同一件事". 抽掉对话, 剩下的字符串会让模型按自己的理解填空.
>
> 新决策见决策 1.
...
**其零, 决策 1 由"项目级"改成"会话级".** 这是本 ADR 最大的一次反转, 连带改了备选方案 A,
§2 目录布局, §5.5 与 §5.6. 触发它的是一个具体问题: 任务做完之后没有任何路径会清活动指针,
于是下一次打开照样显示一份早就完成的计划. 详见决策 1 与 §5.5 的修订说明.
...
> **2026-08-19 修订.** 原文说的是"启动横幅之下加一行". 会话级分区之后那一行**永远是空的**
> —— 新会话的 id 每次都不同, `sessions/<new_id>/plans/` 必然不存在. 真正有内容可说的时刻
> 只有 `/resume`, 所以那一行挪进了 `ResumeCommand`.
>
> 这处是决策 1 改动的连带后果: 一段在旧设计下有意义的展示, 在新设计下成了死代码.

`/resume` 续写一段会话之后加一行, 无内容时整行不显示:
...
> **2026-08-19 重写.** 原文的方案是"记 `updated_at`, 长期未更新时只提示不自动清除, 清除
> 是用户的决定". 那等于**永远不清** —— 没有任何一条路径会去动那个指针, 于是任务早就做完
> 了, 下一次打开照样显示"计划: X (已批准, 5 步)".
>
> 根因不是"没人清理", 是**指针的生命周期定错了**. 决策 1 把作用域收进会话之后, 跨会话
> 那一类陈旧自动消失: 新会话什么都读不到.

会话级之后只剩**会话内**的一类: 一件事做完了, 用户接着提下一件, 而上一份计划还挂着.

处理办法是**推导, 不改盘**. `ActivePlanning` 给出两个派生属性, 提示词块与启动横幅都用
它们, 而 `/plan show`, `/plan list` 这类显式命令仍然读原始状态:

| 属性 | 判据 |
| --- | --- |
| `live_todo` | 有条目, 且**不是**全部落终态 |
| `live_plan` | 还是 `proposed` (等着人裁决), 或者它播种的待办还没做完 |

三条设计理由:

- **`proposed` 单独留一条路.** 那种计划还没有待办 —— 它正等着人拍板, 而那恰恰是最该被
  看见的一份. 按"没待办就不显示"处理会把它藏掉.
- **待办被别的清单顶掉时按做完处理.** 执行跟踪没了, 这份计划也就无从判断做没做完; 方向
  取"少显示一份陈旧的东西".
- **推导而不是改盘.** 加载必须是只读的 (§5.2), 而且文件记录要留给 `/plan list`. "系统
  自动作废一份人批准过的计划"这条顾虑仍然成立 —— 所以这里作废的只是**显示**, 不是状态.

一份做完的待办也不再渲染: 留一份 "5/5 完成" 每轮注入, 对模型是噪音, 对下一件事还是误导
—— 它会去想这两件事有没有关系.
...
会话级分区之后**前后两个会话之间不做交接** —— 那正是决策 1 要的效果. 想接着做上一段的
活, 路径是 `/resume` 回到那个会话, 而不是让计划自己漂过来.
