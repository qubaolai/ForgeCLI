...
sequenceDiagram
    ...
    R->>U: 四选一菜单
    U-->>R: 同意并执行
    R->>R: PlanningService.approve() + set_mode(ACCEPT_EDITS)
...
| 同意并执行 | `decision="approve_and_run"` | 同"同意" | `PLAN -> ACCEPT_EDITS` (仅当前为 PLAN) | 有, 输入为合成文本 |
...
- 非交互环境下计划永远停在 `proposed`, 除非用户事后用斜杠命令处理. 自动化场景需要另行
  设计.
- "同意并执行"只覆盖 `PLAN -> ACCEPT_EDITS` 一条边. 用户想批准后直接进 `auto` 档仍需手动
  切换 —— 这是刻意的, 见决策 4.
