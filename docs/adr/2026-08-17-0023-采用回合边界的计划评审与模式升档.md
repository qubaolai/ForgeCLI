...
> 2026-09-04 修订 (ADR-0043): 本节原本还写着"机制对将来的 `ask_user` 一类工具同样成立".
> **那句话不成立.** 回合边界停顿之后, 那次调用的 `tool_call` 要么已经拿到一条占位结果
> (真回答就只能变成一条新的用户消息), 要么跨着边界欠着一条 (窗口进入一个供应商会拒的
> 状态). 向人提问因此改走阻塞式工具, 回答直接就是那次调用的 `ToolResult`. 本链条确定
> 只有 `plan_write` 一个产出方.
...
- 停顿机制以字段而非工具名表达 (但它没能覆盖向人提问, 见 ADR-0043; 本链条只服务计划
  评审).
