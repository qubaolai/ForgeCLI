...
以下能力在接口上预留，默认排在 MVP 之后：`RateLimiter`、`LlmCacheController`、prompt 缓存标注、
`ProviderHealthRegistry` 熔断、`BudgetGuard` 实裁决、可观测性聚合。MVP 阶段它们以 no-op 或
直通实现存在，不改变调用链；`TokenEstimator` 至少要有近似实现以支撑上下文校验。
