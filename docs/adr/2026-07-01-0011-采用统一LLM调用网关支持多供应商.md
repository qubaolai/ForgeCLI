...
ProviderRequest                      # gateway -> adapter 的内部请求
  thinking: ThinkingConfig           # 由模型目录生成，不来自 ModelRequest
```

规则：

- `temperature`、`top_p`、`max_output_tokens` 等通用参数由 gateway 统一校验。
- thinking mode/effort 是具体模型的运行时默认参数，保存在该模型的 `llm.toml` 条目中；
  切换模型即切换 thinking，不提供请求级显式覆盖。
- `thinking_mode = auto` 表示由 gateway 按 origin 默认策略决定是否开启思考（例如
  `plan`/`review`/`debug` 默认开，`title`/`summary` 默认关），不交给 LLM 决定。
- provider 支持 effort 级别时直接翻译 `thinking_effort`；只支持 token 预算时由 adapter
  把 effort 映射成 budget_tokens。
...
- 每个 `[llm.providers.<provider>.models.<model>]` 可配置 `thinking_mode` 和
  `thinking_effort`；缺省回落 `auto/none`。`ModelRequest` 不允许携带 thinking。
...
- `/config` 可配置当前模型、用途模型覆盖、具体模型的 thinking mode/effort，以及
  应用级 cache / circuit breaker / retry。
- thinking 的 ADR、模型 schema、目录、gateway、菜单与测试口径统一为模型级
  `llm.toml` 字段；`config.toml`、项目 `forge.toml` 和 `ModelRequest` 均不保存 thinking。
- 输入框下方右侧持续显示当前模型和 thinking 配置，配置修改后刷新。
