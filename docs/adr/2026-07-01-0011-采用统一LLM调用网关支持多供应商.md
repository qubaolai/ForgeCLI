...
- 注入超时、重试、temperature、max_tokens 等默认值，并解析具体模型的 thinking。
...
- `params` 只承载调用方可设置的通用超参与 provider 私有选项，不包含 thinking。
  thinking 必须跟随最终解析出的具体模型配置，调用方不得按请求覆盖。
...
ModelCatalogEntry
  thinking_capabilities
    supported: bool
    efforts: string[]                 # 由具体模型声明的开放集合
    default_effort: string?
  thinking_settings
    mode: on | off
    effort: string?

ProviderRequest                      # gateway -> adapter 的内部请求
  thinking
    enabled: bool                     # mode 已由 gateway 解析
    effort: string?                   # 模型的有效强度
```

规则：

- `temperature`、`top_p`、`max_output_tokens` 等通用参数由 gateway 统一校验。
- thinking mode、开放 effort 集合与模型默认 effort 都跟随具体模型，保存在该模型的
  `llm.toml` 条目中；`/thinking` 的当前会话覆盖只保存在进程内。切换模型即切换
  thinking 默认值，不提供请求级显式覆盖。
- mode 是封闭集合 `on | off`；effort 是模型声明的开放字符串集合。不同模型可以
  分别声明 `high/max`、`low/high/max` 或 `low/medium/high/max/xhigh`，gateway 不维护
  跨供应商固定强度枚举。
- `off` 只由 mode 表达，不再使用 `effort = none`；切换为 off 时保留用户此前选择的
  effort，下次重新开启时继续使用。
- `thinking_mode` 只表达 `on` 或 `off`；是否启用不依赖 origin，也不交给 LLM 决定。
- provider 支持 effort 级别时原样翻译模型的有效 effort；只支持 token 预算时由 adapter
  把已知 effort 映射成 budget_tokens，无法映射的开放等级必须返回明确错误。
- 请求声明需要 thinking 时，resolver 直接检查模型的 mode；`off` 返回明确能力错误，
  不能静默忽略或自动选择其他模型。
...
目录来源：`ModelCatalogService` 以内置目录为基线，`[llm.providers.*.models.*]`（§16）中用户配置的
...
通用参数合并优先级（高到低）：请求级 `ModelParams` > 按 `origin` 的系统默认参数 >
provider 默认（timeout、max_retries）。thinking 不参与请求参数合并，始终读取解析后模型的
`thinking_mode` / `thinking_effort`。`provider_options` 按 provider 命名空间整体透传。
...
当前只实现**环境变量**一种来源。credential reference 就是**裸环境变量名**
（如 `DEEPSEEK_API_KEY`），不带 scheme 前缀——前缀原本是多来源分发键，来源
收敛为单一后无消歧义作用，已去除（2026-07-17 复核）。ForgeCLI 是个人单用户
CLI，环境变量足以覆盖本地使用；ADR-0012 §5 曾扩展 `dotenv:` / `keychain:`
两种 scheme，2026-07-17 复核后判定不需要并整体移除（个人使用下 dotenv 相对
直接 `export` 无实质收益、keychain 增加平台耦合与实现面），`CredentialResolver`
回到 env-only 且无前缀。企业托管凭证 / 系统 keychain / secret store 作为
**企业版**候选来源登记在 [后续迭代 Backlog](../roadmap/backlog.md)，不在当前范围。
...
项目级 `projects/<project-id>/forge.toml` 只保存当前模型与可选的用途模型覆盖：
...
应用级 provider/model 与网关运行时配置保存到 `llm.toml`：
...
credential_refs = ["DEEPSEEK_API_KEY", "DEEPSEEK_API_KEY_BACKUP"]  # 环境变量名，无前缀
timeout = 60
max_retries = 2
...
- `[model].provider` 和 `[model].name` 表示当前项目的主 Agent 模型。
- 系统默认让全部 origin 使用当前模型；`[model_overrides.<origin>]` 可为特定用途指定
  provider/model，用户可在项目配置中新增、修改或删除该覆盖。
- `[llm.providers]` 表示可调用模型目录和 adapter 参数；`[llm.cache]`、
  `[llm.circuit_breaker]`、`[llm.retry]` 是应用级网关运行时策略，完整语义见 ADR-0012。
- 每个模型可声明 `thinking_efforts`、`thinking_default_effort`、`thinking_mode` 和
  `thinking_effort`。不存在额外的 thinking 支持标记：mode 直接决定是否启用，effort
  相关字段直接决定可用强度。模型目录完全来自 `llm.toml`，代码不内置具体模型；未声明
  mode 时默认 `off`。`ModelRequest` 不允许携带 thinking。
- provider 可以使用单个 `api_key_env`，也可以使用多个 `credential_refs` 支撑 key pool。
- `/config` 通过 application service 编辑以上配置，CLI 不直接解析或写 TOML。
- 交互式 CLI 在输入框下方右侧持续显示“当前项目模型 + 该模型的 thinking”；
  `/model` 或 `/config` 修改后下一次渲染立即反映。
...
- thinking 配置按模型目录 capability 校验。
- 未配置凭证时不发起真实网络请求。
...
- `/config` 可编辑任意已配置模型的 thinking 开放强度集合、默认强度与 mode，
  不提供独立支持字段或当前 effort 编辑项；mode 直接控制 thinking，`off` 只关闭使用
  并保留强度配置。当前 effort 仅由 `/thinking` 在进程内原子修改，不写入配置文件。
  `/config` 还可配置当前模型、用途模型覆盖及应用级 cache / circuit breaker / retry。
- thinking 默认值的 ADR、模型 schema、目录、gateway、菜单与测试口径统一为模型级
  `llm.toml` 字段；`/thinking` 覆盖只存在进程内，`config.toml`、项目 `forge.toml`
  和 `ModelRequest` 均不保存当前 thinking。
- 输入框下方右侧持续显示当前模型和 thinking 配置；`/config` 现读 `llm.toml`，
  `/thinking` 与网关、状态行共享进程内 override，修改后立即刷新。
- `/thinking` 支持查看、切换 `on|off`、修改模型支持的开放 effort；非法等级不得
  部分更新进程状态。关闭时状态行只显示 `off`，但本次进程内保留此前 effort。
...
- `docs/adr/2026-06-17-0005-项目配置系统设计.md`、`docs/adr/2026-06-23-0008-区分应用配置与项目配置存储.md`
  — §16 配置形态：当前模型与用途模型覆盖为项目级；thinking 跟随应用级模型目录中的
  具体模型条目；网关运行时策略为应用级。
