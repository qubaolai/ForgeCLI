...
```text
domain/human_prompt.py        # HumanPrompt / PromptChoice / PromptKind / PromptAnswer
                              # 纯数据. 不认识审批, 也不认识提问
application/human_prompt.py   # HumanPromptService (ABC): ask(HumanPrompt) -> PromptAnswer
                              #   release_pending(note) / begin_turn() / close()
                              # PendingHumanPromptService: 无人可答时的实现
interfaces/runtime/human_prompt.py  # BlockingHumanPromptBroker: 全局唯一的待答队列
...
- **`interfaces/runtime/approval.py` 被 `interfaces/runtime/human_prompt.py` 顶替**
...
1. **`HumanPrompt` 必须保持哑.** 里面不许出现 `ApprovalScope`, `ToolPlan`,
   ...
   提问也拿到它. `scripts/check_arch.py` 加一条断言: `domain/human_prompt.py` 与
   `application/human_prompt.py` 不 import `domain.security.*`.
...
```text
domain/human_prompt.py               # HumanPrompt / PromptChoice / PromptKind / PromptAnswer
application/human_prompt.py          # HumanPromptService (ABC) + PendingHumanPromptService
application/tools/builtin/ask_user.py  # AskUserTool(HumanPromptService)
interfaces/runtime/human_prompt.py   # BlockingHumanPromptBroker
```
...
- `domain/human_prompt.py` 与 `application/human_prompt.py` 不 import `domain.security.*`
  —— `scripts/check_arch.py` 加一条断言.
- `interfaces/runtime/approval.py` 已被 `human_prompt.py` 顶替, 且 `interfaces/web/` 与
