...
**不进入本表（均属 MVP，在后续滚动切片实现）：**

- LLM 网关：streaming 流式输出、tool calling、prompt / 响应缓存。
- Agent 主功能：Agent 主循环（受控 ReAct）、模式策略、工具系统与审批、上下文压缩与记忆、Sub-Agent。

这些是 ForgeCLI 的核心能力，MVP 必须实现，只是不在当前 LLM 网关滚动切片内，应安排到后续 MVP 滚动计划，而不是登记为后续大版本 backlog。
...
来源：[ADR-0011](../adr/2026-07-01-0011-采用统一LLM调用网关支持多供应商.md)。MVP 网关已实现非流式 `complete`、streaming、tool calling、缓存、路由、凭证 / 计量 / 错误边界；下列治理能力 MVP 以 no-op 或直通预留，排在 MVP 之后。
