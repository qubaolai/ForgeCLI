...
- [06-code-reading-guide.md](06-code-reading-guide.md)：代码阅读路线，按三条主线（对话链路、工具裁决管线、Shell 分析）给出通读顺序、配套 ADR、自检问题与已知缺口。
...
- [ADR-0023：采用回合边界的计划评审与模式升档](adr/2026-08-17-0023-采用回合边界的计划评审与模式升档.md)：定义工具声明的回合边界停顿、补充/拒绝/同意/同意并执行四选一，以及 `PLAN → AUTO` 的显式升档边界（升档终点由 ADR-0038 从 `ACCEPT_EDITS` 改为 `AUTO`）。
- [ADR-0024：采用已证明只读效果的 Shell 命令快速放行](adr/2026-08-19-0024-采用已证明只读效果的Shell命令快速放行.md)：定义按已证明效果而非能力上界放行只读 Shell 命令的八条判据、失效方向，以及必须排在参数模型之后的实现顺序。
...
- [ADR-0048：采用运行状态一致性与模块职责收敛方案](adr/2026-09-06-0048-采用运行状态一致性与模块职责收敛方案.md)：总体优化提案，说明会话状态、事件重连、取消等待、质量门禁、模块职责、两入口一致性及性能治理的现状、方案和收益。
