...
flowchart TD
    ...
    N -->|某条会话| Q["onResume → POST /sessions/{id}/resume，见 §12.2"]
    Q --> R{"会话属于当前激活项目?"}
    R -->|是| S["正常恢复"]
    R -->|否| T["active_runtime.resume 在自己的 sessions 目录找不到 → SessionStateError → 404"]
```
...
要守住的边界：`resume` 仍然只打**当前激活**的运行时（附录 A.3），列表里非当前项目的会话
点下去会 404，得先点项目行激活。当前代码没有在前端拦这一步，也没有"激活并恢复"的合并
接口；读到这里不要把"能看到"当成"能直接打开"。
...
10. **源码中的历史注释也可能过时**：例如描述已删除的分类器、去重或旧命令名的注释，必须与
    实际函数体、装配点和消费方交叉核对。
11. **侧栏列出的会话不等于可直接恢复的会话**：按项目列会话不激活项目，而恢复只打当前激活
    的运行时；非当前项目的会话要先激活项目，见 §12.6。
