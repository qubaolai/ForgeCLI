...
fs_edit_file"就不再是关于真实目录的陈述, 而只是一句好听的话.
"""
...
def _run(mode: SessionMode) -> str:
    ...
    registry.register_all(
        (
            _Tool("fs_read", Capability.WORKSPACE_READ),
            _Tool("fs_edit_file", Capability.WORKSPACE_WRITE),
            _Tool("shell_run", Capability.EXECUTE_SHELL),
        )
    )
...
def test_plan_mode_hides_mutating_tools() -> None:
    ...
    assert "fs_read" in visible
    assert "fs_edit_file" not in visible
    assert "shell_run" not in visible


def test_hidden_tools_are_listed_as_hidden_not_omitted() -> None:
    """ "看不见 fs_edit_file"与"没有这个工具"是两回事, 混在一起会被当成功能缺失."""
    text = _run(SessionMode.PLAN)

    hidden = _section(text, "本模式下不可见")
    assert "fs_edit_file" in hidden
    assert "shell_run" in hidden
...
def test_other_modes_see_everything() -> None:
    ...
    for name in ("fs_read", "fs_edit_file", "shell_run"):
        assert name in text
