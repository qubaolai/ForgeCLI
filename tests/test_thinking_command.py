...
def _command(
    tmp_path: Path,
) -> tuple[ThinkingCommand, ConfigService, LlmConfigService, _RecordingOutput]:
    ...
    return ThinkingCommand(config, llm, output), config, llm, output
...
def test_missing_current_model_has_actionable_message(tmp_path: Path) -> None:
    ...
    llm = LlmConfigService(TomlLlmConfigStore(tmp_path / "llm.toml"))
    output = _RecordingOutput()
    command = ThinkingCommand(config, llm, output)
