...
from forgecli.application.tools import builtin
...
def all_tool_specs() -> tuple[ToolSpec, ...]:
    """遍历 builtin 包收集全部 ToolSpec, 按名字排序.

    不手写一张工具清单: 那是第二份要同步的表, 而漏登记的那个工具恰恰是没人检查的那个 ——
    "每个工具都得满足 X"这类用例, 一旦清单漏了一项就静默变成"每个**登记过的**工具".

    两种声明方式都要收: 八个工具把 `_SPEC` 放模块级, 计划与记忆那几个放类属性.
    """
...
def prompt(instructions: tuple[ProjectInstruction, ...] = ()) -> PromptSnapshot:
    """给测试用的真提示词快照.
    ...
    ADR-0042 之后它只收项目指令: mode, 工具目录, 计划待办与记忆都已经不是提示词的输入.
    """
    return SystemPromptBuilder().build(instructions)
...
def assembled(
    *,
    ...
    """一份真的分层上下文 (ADR-0041 决策 1).

    走真的渲染路径而不是手填字符串: 层与层之间的顺序正是被测的东西, 假数据测不出来.
    """
    return AssembledContext(
        policy=prompt(),
        runtime_context=render_runtime_context(FACTS, mode=mode, fence=None),
