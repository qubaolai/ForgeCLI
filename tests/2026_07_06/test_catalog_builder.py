"""build_catalog：模型目录完全由用户配置构建（ADR-0011 §4）。"""
...
def test_no_models_exist_without_user_config(tmp_path: Path) -> None:
    catalog = build_catalog(_config(tmp_path))
    assert (
        catalog.has_model(ModelRef(provider="deepseek", model="deepseek-chat")) is False
    )


def test_user_config_defines_model_window(tmp_path: Path) -> None:
    content = """
...
extra = { supports_tools = true, supports_json_schema = true }
"""
    catalog = build_catalog(_config(tmp_path, content))
...
"""
    ...
    assert entry.effective_thinking_effort == ThinkingEffortName("high")
