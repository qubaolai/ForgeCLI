...
Follow the daily plan in `docs/roadmap/mvp`. Prefer guidance, review, and validation over unrequested implementation. Keep changes scoped to the current phase and update docs when behavior or architecture decisions change. There are two entry points over one composition root: bare `forge` starts the local Web control plane (ADR-0025), and `forge --cli` starts the interactive terminal session (ADR-0045). Both assemble through `interfaces/runtime`; neither `interfaces/web` nor `interfaces/tui` may grow business logic. Any setting reachable from one entry point must be reachable from the other, and both must render from the same declaration (`config_keys.SCHEMA`, `STANDARD_FIELDS`, `domain/intents` stance tables) rather than hand-written copies.
