...
**输出形态跟着 pattern 走, 不加参数** (ADR-0029 规则一: 工具可以变聪明, 不该变宽):

    不给 pattern  ->  树视图. 你还不知道要找什么, 在认识这个目录.
    给了 pattern  ->  扁平清单. 你知道要找什么, 在定位它.
...
_SPEC = ToolSpec(
    ...
    description=(
        "在一个目录里定位文件, 或者认识它的结构.\n"
        "不传 pattern 时按层级渲染目录树, 用来快速认识不熟悉的代码库; "
        "默认从工作区根开始展开 3 层, 深度到头的目录只报还有多少项.\n"
        "传了 pattern 时按 glob 返回扁平文件清单, pattern 相对给定目录展开: "
        "'*' 只匹配当前一层, '**/' 前缀递归整棵树 —— "
...
class FindTool(Tool):
    ...
    def prepare(
        ...
        if isinstance(raw_pattern, str) and raw_pattern.strip():
            return self._locate(
                request, context, facts.realpath, raw_pattern, include_ignored
            )
    ...
    def _locate(
        ...
        raw_matches = context.filesystem.expand_glob(
            pattern, root=root, max_results=_MAX_GLOB_CANDIDATES + 1
        )
    ...
    def perform(
        ...
        if str(data.get("mode")) == "tree":
            ...
            body = joined(listed) or _empty_list_message(root, str(data["pattern"]))
            notices = []
...
def _empty_list_message(root: str, pattern: str) -> str:
    return (
