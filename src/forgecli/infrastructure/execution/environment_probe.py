...
只探测, 不改变宿主, 不装东西, 不联网, 不改配置; 探测到什么就报告什么, 隔离等级
目前恒为 NO_SANDBOX, 因为本次没有实现沙箱层, 而不是"探测不出来先当强隔离用".

受控 PATH 构造原则: 只收标准系统目录与用户显示配置的工具链目录, 不含`.`, 不含
工作区, 不好临时目录, 不含 node_modules/.bin等 那些目录 Agent 自己能写.
"""
...
def probe_execution_profile(
    *,
    protected_root_hash: str,
    toolchain_dirs: Sequence[str] = (),
) -> ExecutionProfile:
    """按当前平台生成执行快照
    
    toolchain_dirs 是用户现实配置的工具链目录(例如项目虚拟环境) 他们会进 PATH,
    但 ADR-0014 §4.2 要求把这类目录标记为可写/不可信; 标记发生在可执行文件身份解析
    那一步(阶段 4), 这里只负责如实把他们放进快照, 让 profile hash 能反映这个选择.
    """
    ...
    return ExecutionProfile(
        ...
        protected_roots_hash=protected_root_hash,
    )

def build_execution_environment(
    profile: ExecutionProfile, raw: Mapping[str, str] | None = None,
) -> dict[str, str]:
    """净化环境. 默认基于 os.environ, 测试可以传入固定映射."""
