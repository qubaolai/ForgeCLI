...
class TurnDisposition(Enum):
    ...
    **只能缩短一次 turn, 不能延长, 不能授予能力.** 失效方向朝安全 —— 将来一个行为不端的
    ...
    循环按这个**字段**分流, 不去读 content 里的文字, 也不认识任何工具名.

    ADR-0023 当初预言这套机制"对将来的 ask_user 一类工具同样成立", 而 ADR-0043 的结论是
    **不成立**: 回合边界停顿之后, 那次调用的 tool_call 要么已经拿到一条占位结果, 要么
    跨着边界欠着一条 —— 两种都不是"回答就是这次调用的结果". 提问因此走阻塞式工具, 不走
    这条链. 于是这个字段确定只有 `plan_write` 一个产出方.
    """
