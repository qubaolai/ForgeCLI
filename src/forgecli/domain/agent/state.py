...
class AssembledContext:
    """本轮发给模型的上下文, 按**变更源**分层 (ADR-0041 决策 1).
    ...
    整条请求的**最后一个内容块**, 后面没有任何东西会被它作废. **用户输入不是独立一层**
    ...
    预算是**可选**的, 与必填的 policy 不同: 缺提示词必须让调用被阻止, 而缺预算只是不
    ...
    def to_request_messages(
        ...
        状态帧**不进窗口**: 它每轮重建, 进了窗口就意味着下一轮的历史里躺着一份过时的
        副本, 而摘掉它又是一次中段改写.
        """
