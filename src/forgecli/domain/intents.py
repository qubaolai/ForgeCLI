...
class ApprovalPolicy(Enum):
    ...
    #: 只放行 Forge 看得清的那一类: 命令解析完整, 目标集合封闭, 且触达的东西都在围栏内
    #: (ADR-0046). 看不透要跑什么的 —— 脚本正文, `npm test`, `find -exec`, 联网 ——
    #: 仍要点头. 判据是**这条命令碰了什么**, 不是"它是不是一次 shell 调用".
    ALWAYS = "always"
    #: 围栏内不必先看清楚: 看不透的执行也自动放行. 越界的仍然要问.
    AUTO = "auto"
