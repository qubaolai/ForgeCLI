...
正式实现时只描述 intent 的领域语义，不在这里耦合 Typer 参数或 REPL 输入格式。
设计约束:
    - 不可变: 全部使用 frozen dataclas
    - 值相等: dataclass 自动按字段比较, 不同子类型互不相等
    - 构造即有效: 非法组合在类型层面不可表达; 剩余不变量在 __post_init__ 校验
    - 无副作用: 只承载数据与纯查询, 不做解析, 不对接llm, 不执行工具

解析职责属于 IntentRouter: 它负责识别原始输入为下面的某个具体子类型
"""
...
class UserIntent(ABC):
    ...
    请勿直接数理化: 应使用下方的具体子类型. raw_text 为用户原始输入.
    所有意图都携带, 用于回显、日志和测试
    """
    raw_text: str
...
class SlashCommand(UserIntent):
    """已识别的查询型斜杠命令, 如 /help /status
    
    command 为去掉前导斜杠的规范名(小写); args 为不可变参数序列
    模式切换与会话控制不走这里, 分别由 ModeChange / ControlSignal 表达
    """
