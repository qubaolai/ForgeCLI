...
原本在 application/llm/gateway/schema_validation.py, 但它跟 LLM 没有关系:
ADR-0004 §3 要求 ToolSpec.input_schema 在运行前强制校验, 工具系统不该为了一个纯函数
去 import LLM 网关。移到 shared, 两侧共用。

原用途（ADR-0011 §3.7）：

标准库实现的 JSON Schema 子集，覆盖内部结构化任务常用的约束：
...
def validate_json_schema(
    ...
    """校验 data 是否满足 schema 子集, 返回错误消息列表, 为空则表示通过"""
    errors: list[str] = []
    ...
    if isinstance(enum, list) and data not in enum:
        errors.append(f"{path}: 取值不再枚举允许范围内: {data!r}")
    ...
    if isinstance(data, dict):
        ...
        if isinstance(required, list): # 必填检查
            for name in required:
                if name not in data:
                    errors.append(f"{path}: 缺少必填字段 {name!r}")
        for name, value in data.items():
            sub_schema = properties.get(name)
            if isinstance(sub_schema, Mapping): # 递归检查map
                errors.extend(
                    validate_json_schema(value, sub_schema, path=f"{path}.{name}")
                )
            elif schema.get("additionalProperties") is False:
                errors.append(f"{path}: 不允许额外的未知属性 {name!r}")

    # json数组
    if isinstance(data, list):
        items = schema.get("items")
        if isinstance(items, Mapping):
            for index, item in enumerate(data): # 递归检查map
                errors.extend(
